...
function renderCases() {
  ...
  pageSubtitle.textContent = "Selecciona un caso y responde con retroalimentación inmediata.";
  root.innerHTML = `
    <section class="two-col">
      <div class="panel">
      ...
      <div class="panel" id="case-panel">
        ...
        <p>El panel mostrará el enunciado, opciones, temporizador y retroalimentación.</p>
      </div>
    </section>
  `;
  ...
  function draw(filter = "") {
    ...
    list.innerHTML = data.cases.filter(c =>
      [c.title, c.block, c.specialty, c.statement, ...(c.tags || [])].join(" ").toLowerCase().includes(q)
    ).map(c => `
    ...
    `).join("");
    list.querySelectorAll(".case-card").forEach(btn => btn.addEventListener("click", () => openCase(Number(btn.dataset.id))));
  }
